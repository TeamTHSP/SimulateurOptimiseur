...
int lectureFichierParam(char *nomFichierParam)
{
	...
	if (fParam != NULL)
	{
    	...
    	while (! feof(fParam))
    	{
    		...
    		paramAppli.listeDevise[nbDevises].departMoyMin 		= atoi(ligne[10]);
    		paramAppli.listeDevise[nbDevises].departMoyMax 		= atoi(ligne[11]);
    		paramAppli.listeDevise[nbDevises].relacherMoyMin 	= atoi(ligne[12]);
    		paramAppli.listeDevise[nbDevises].relacherMoyMax 	= atoi(ligne[13]);
    		paramAppli.listeDevise[nbDevises].ecartSSMin 		= atoi(ligne[14]);
    		paramAppli.listeDevise[nbDevises].ecartSSMax 		= atoi(ligne[15]);
    		paramAppli.listeDevise[nbDevises].creationBD 		= ligne[16];
    		paramAppli.listeDevise[nbDevises].optimiserGain		= ligne[17];
    		paramAppli.listeDevise[nbDevises].optimiserSL		= ligne[18];
    		paramAppli.listeDevise[nbDevises].optimiserMOY		= ligne[19];
    		paramAppli.listeDevise[nbDevises].optimiserSS 		= ligne[20];
    		paramAppli.listeDevise[nbDevises].afficher 	 		= ligne[21];
			...
			for (int month = 0; month < MOIS ; month++)
			{
				for (int day = 0; day < JOUR ; day++)
				{
					...
					for (int min = MINUTE_DEBUT + deltaMin; min <= minuteFin + deltaMin ; min++)
					{
						...
						if(open >= pivotAjuste) // Cas d'une vente
						{
							...
							if (minuteCourante.low < takeProfit )
							{
								...
								tpAtteint = 1;
								#ifdef DEBUG_SL
									fprintf(fd, "%d/%d/%d;%.2f;0;%d;0;1\n", day+1, month+1, year, (open - takeProfit) * MONTANT / open, min);
								#endif
								break;
							...
							else if(minuteCourante.high > takeLoss )
							{
								...
								slAtteint = 1;
								#ifdef DEBUG_SL
									fprintf(fd, "%d/%d/%d;%.2f;0;%d;1;0\n", day+1, month+1, year, (open - takeLoss) * MONTANT / open, min);
								#endif
								break;
							}
						...
						else // cas d'un achat
						{
							...
							if (minuteCourante.high > takeProfit )
							{
								...
								tpAtteint = 1;
								#ifdef DEBUG_SL
									fprintf(fd, "%d/%d/%d;%.2f;0;%d;0;1\n", day+1, month+1, year, (takeProfit - open) * MONTANT / donneeMinute->open, min);
								#endif
								break;
							}
							else if(minuteCourante.low < takeLoss )
							{
								...
								#ifdef DEBUG_SL
									fprintf(fd, "%d/%d/%d;%.2f;0;%d;1;0\n", day+1, month+1, year, (takeLoss - open) * MONTANT / donneeMinute->open, min);
								#endif
								break;
							}
						}
					...
					if(slAtteint == 0 && tpAtteint == 0)
					{
						...
						#ifdef DEBUG_SL
							fprintf(fd, "%d/%d/%d;%.2f;0;0;0;0\n", day+1, month+1, year, (diffOpenClose != 0) ? diffOpenClose * MONTANT / open : 0);
						#endif
					}
					...
					for (int day = 0; day < JOUR ; day++)
					{
						...
						for (int min = MINUTE_DEBUT + deltaMin; min <= minuteFin + deltaMin ; min++)
						{
							...
							if(open >= pivotAjuste) // Cas d'une vente
							{
								...
								gainVente 	= tpv/100.0;
								takeProfit 	= open * (1.0 - gainVente / 100.0) ;
								takeMoy 	= open + departMoy/10000.0 ; // en pips
								releaseMoyPrice	= takeMoy - relacherMoy/10000.0;
								...
								{
									gainOrdre = relacherMoy/10000.0 * MONTANT / releaseMoyPrice - SPREAD_PRICE;
									gainJournee += gainOrdre;
									moyActive = 0;
									#ifdef DEBUG_MOY
